...
public abstract class SqlTypeUtil {
   ...
   */
  public static boolean isCharTypeComparable(List<RelDataType> argTypesWithAny) {
    ...
    for(RelDataType t : argTypesWithAny){
      if(t.getFamily() != SqlTypeFamily.ANY) argTypes.add(t);
    }
    if(argTypes.size() < 2) return true;
  ...
  public static boolean canAssignFrom(
    ...
    if(isAny(toType) || isAny(fromType)) return true;
  ...
  public static boolean areCharacterSetsMismatched(
    ...
    if(isAny(t1) || isAny(t2)) return false;
